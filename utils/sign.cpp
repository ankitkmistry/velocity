...
public:
    ...
    char peek() {
        if (i >= text.size())throw SignatureError(text);
        int j = i;
    ...
    SignatureError error(string msg) {
        return SignatureError(text, msg);
    }
};
...
static vector<T> slice(vector<T> list, int64 start, int64 end) {
    ...
    if (start > end)throw FatalError("slice(): start must be less than end");
    vector<T> result = {};
